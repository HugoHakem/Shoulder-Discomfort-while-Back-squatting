...
H1=0.37; % Forearm (m)
H2=0.30; % Humerus (m)
c=0.025; % Half of the side of the shoulder associated to a cube (m)
bheta=4; % Fixed angle in degrees between forearm and humerus in the YX plan
Bheta=bheta*pi/180; % Fixed angle in Radian between forearm and humerus in the YX plan
...
figure;
%subplot(1,2,1)
im=imagesc(DeltaHand,DeltaH,FM);
...
%% Plot Mx Mz
subplot(1,2,1)
im=imagesc(DeltaHand,DeltaH,Mx);
title('Joint reaction torque Mx (N.m)')
xlabel('DeltaHand (m)')
ylabel('DeltaH (m)')
set(im,'alphadata',(~isnan(abs(Mx))))
colorbar

subplot(1,2,2)
im=imagesc(DeltaHand,DeltaH,abs(Mz));
title('Joint reaction torque Mz (N.m)')
xlabel('DeltaHand (m)')
ylabel('DeltaH (m)')
set(im,'alphadata',~isnan(Mz))
colorbar
...
%% Plot Fzx oriantation Nothing really interesting, keep being around 14 degrees
figure
subplot(1,2,1)
